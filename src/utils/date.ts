...
function getLocale(options?: DateOptions) {
    return options?.locale ?? 'en-US';
}

export const date = {
    // Format a Date or ISO string to "Jan 5, 2025"
    formatShort(d: Date | string, options?: DateOptions) {
    ...
    formatLong(d: Date | string, options?: DateOptions) {
        const dt = typeof d === 'string' ? new Date(d) : d;
        return new Intl.DateTimeFormat(getLocale(options), {
            year: 'numeric',
            month: 'long',
    ...
    // "3:45 PM"
    formatTime(d: Date | string, options?: DateOptions) {
        const dt = typeof d === 'string' ? new Date(d) : d;
        return new Intl.DateTimeFormat(getLocale(options), {
            hour: 'numeric',
            minute: '2-digit',
